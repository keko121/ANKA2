...
#define STRNCPY(dst, src, len)          do {strncpy(dst, src, len); dst[len] = '\0'; } while(0)
    ...
    extern char *	str_dup(const char * source);	// ¸?¸?¸® ÇÒ´ç ÇØ¼­ source º¹»ç ÇÑ°Å ¸®ÅÏ
    extern void		printdata(const unsigned char * data, int bytes); // data¸¦ hex¶û ascii·Î Ãâ·Â (Æ?Å¶ º?¼® µî¿¡ ¾²ÀÓ)
    extern int		filesize(FILE * fp);	// ÆÄÀÏ Å©±â ¸®ÅÏ

#define core_dump()	core_dump_unix(__FILE__, __LINE__)
    extern void		core_dump_unix(const char *who, WORD line);	// ÄÚ¾î¸¦ °­Á¦·Î ´?ÇÁ
...
#define TOKEN(string) if (!str_cmp(token_string, string))
    ...
    extern int		number_ex(int from, int to, const char *file, int line); // fromÀ¸·Î ºÎÅÍ to±îÁöÀÇ ·£´? °ª ¸®ÅÏ
#define number(from, to) number_ex(from, to, __FILE__, __LINE__)
    ...
    extern DWORD	thecore_random();				// ·£´? ÇÔ¼ö
