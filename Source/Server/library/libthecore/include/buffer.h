...
#define SAFE_BUFFER_DELETE(buf)		{ if(buf != NULL) { buffer_delete(buf); buf = NULL; } }
	...
	extern LPBUFFER	buffer_new(int size);				// »õ ¹öÆÛ »?¼º
    ...
    extern void		buffer_read(LPBUFFER buffer, void * buf, int bytes);		// ¹öÆÛ¿¡¼­ À?´Â´Ù.
    ...
    // buffer_proceed ÇÔ¼ö´Â buffer_peekÀ¸·Î À?±â¿ë Æ÷ÀÎÅÍ¸¦ ¸®ÅÏ ¹?¾Æ¼­ ¾µ ÇÊ¿ä°¡
    // ÀÖÀ» ¶§ Ã³¸®°¡ ³¡³ª¸é ¾ó¸¶³ª Ã³¸®°¡ ³¡³µ´Ù°í Åëº¸ÇØ¾ß ÇÒ ¶§ ¾´´Ù. 
    // (buffer_read, buffer_get_* ½Ã¸®ÁîÀÇ °æ¿ì¿¡´Â ¾Ë¾Æ¼­ Ã³¸®µÇÁö¸¸ peekÀ¸·Î Ã³¸®ÇßÀ»
    //  ¶§´Â ±×·¸°Ô µÉ ¼ö°¡ ¾øÀ¸¹Ç·Î)
    extern const void *	buffer_read_peek(LPBUFFER buffer);				// À?´Â À§Ä¡¸¦ ¸®ÅÏ
