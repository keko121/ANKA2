...
typedef struct regen
{
	...
	regen() : prev(NULL), next(NULL), lMapIndex(0), type(0), sx(0), sy(0), ex(0), ey(0), z_section(0),
		direction(0), time(0), max_count(0), count(0), vnum(0), is_aggressive(0), event(NULL), id(0) {}
} REGEN;

EVENTINFO(regen_event_info)
{
	LPREGEN regen;
	regen_event_info() : regen( 0 ) {}
};
...
EVENTINFO(dungeon_regen_event_info)
{
	...
	dungeon_regen_event_info() : regen( 0 ), dungeon_id( 0 ) {}
};
