...
namespace
{
	struct FNotice
	{
		FNotice(DWORD id, bool big, const char * psz) : m_id(id), m_big(big), m_psz(psz) {}
		...
		void operator() (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (ch->IsPC())
				{
					if (m_big == true)
					{
						ch->LocaleChatPacket(CHAT_TYPE_BIG_NOTICE, m_id, m_psz);
					}
					else
					{
						ch->LocaleChatPacket(CHAT_TYPE_NOTICE, m_id, m_psz);
					}
				}
			}
		}
...
void CDungeon::Notice(DWORD id, const char* msg, bool big)
{
