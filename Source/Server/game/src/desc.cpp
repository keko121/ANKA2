...
#define DEFAULT_PACKET_BUFFER_SIZE 95536 // Fix 65536
...
BYTE DESC::GetLanguage()
{
	TAccountTable& rTable = GetAccountTable();

	if (rTable.login)
	{
		BYTE bLanguage = rTable.bLanguage;
		if (bLanguage)
	...
	else
	{
		...
		else
		{
			...
			if (buffer_has_space(m_lpOutputBuffer) < iSize + 8)
			{
				...
				if (buffer_has_space(m_lpOutputBuffer) < iSize + 8)
				{
					const std::string name = GetCharacter() ? GetCharacter()->GetName() : GetHostName();
					const auto header = *(static_cast<const BYTE*>(c_pvData));
					sys_err("buffer mem_size overflow : memsize(%u) write_pos(%u) iSize(%d) m_iPhase(%u) header(%u) who(%s)", m_lpOutputBuffer->mem_size, m_lpOutputBuffer->write_point_pos, iSize, m_iPhase, header, name.c_str());
					m_iPhase = PHASE_CLOSE;
				}
			}
#else
...
void DESC::DisconnectOfSameLogin()
{
	if (GetCharacter())
	{
		...
		GetCharacter()->LocaleChatPacket(CHAT_TYPE_INFO, 103, "");
		DelayedDisconnect(5);
	}
	else
	{
...
void DESC::LocaleChatPacket(BYTE type, DWORD id, const char* format, ...)
{
...
void DESC::LocaleWhisperPacket(BYTE type, DWORD id, const char* namefrom, const char* format, ...)
{
	char chatbuf[256];
	va_list args;
