...
EVENTINFO(tag_DragonLair_Collapse_EventInfo)
{
	...
	tag_DragonLair_Collapse_EventInfo() : step( 0 ) , pLair( 0 ) , InstanceMapIndex( 0 ) {}
};
...
EVENTFUNC( DragonLair_Collapse_Event )
{
	...
	if (0 == pInfo->step)
	{
		SendLocaleNotice(CHAT_TYPE_NOTICE, 0, pInfo->InstanceMapIndex, 600, "%d", pInfo->pLair->GetEstimatedTime());
