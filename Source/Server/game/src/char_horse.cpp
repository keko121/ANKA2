...
bool CHARACTER::StartRiding()
{
...
#ifdef ENABLE_MOUNT_NPC_APPEARANCE_FIX
	if (IsRiding())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("at_binek_engel"));
		return false;
	}
#endif
	...
	if (IsPolymorphed())
	{
		LocaleChatPacket(CHAT_TYPE_INFO, 94, "");
		return false;
	}
	...
	{
		if (GetHorseLevel() <= 0)
			LocaleChatPacket(CHAT_TYPE_INFO, 96, "");
		else if (GetHorseHealth() <= 0)
			LocaleChatPacket(CHAT_TYPE_INFO, 97, "");
		else if (GetHorseStamina() <= 0)
			LocaleChatPacket(CHAT_TYPE_INFO, 98, "");
...
void CHARACTER::HorseSummon(bool bSummon, bool bFromFar, DWORD dwVnum, const char* pPetName)
{
	if ( bSummon )
	{
		...
		if (!m_chHorse)
		{
			LocaleChatPacket(CHAT_TYPE_INFO, 99, "");
			return;
		}
