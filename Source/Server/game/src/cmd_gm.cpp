		...
		case 8:
			{
				...
				tch->UpdateAlignment(amount - tch->GetRealAlignment());	//GM'lerin /set karakterad� alignment ile derece verdikleri komutun hata d�zeltmesidir.
			}
			break;
...
// Empire bazl� bot spawn komutlar�
ACMD(do_BotSpawnA)  // K�rm�z� Krall�k (Shinsoo)
{
	if (!ch)
		return;
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "K�rm�z� Krall�k (Shinsoo) botlar� olu�turuluyor...");
	CBotCharacterManager::instance().BotSpawnShinsoo(ch, spawn_count);
}

ACMD(do_BotSpawnB)  // Sar� Krall�k (Chunjo)
{
	if (!ch)
		return;
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "Sar� Krall�k (Chunjo) botlar� olu�turuluyor...");
	CBotCharacterManager::instance().BotSpawnChunjo(ch, spawn_count);
}

ACMD(do_BotSpawnC)  // Mavi Krall�k (Jinno)
{
	if (!ch)
		return;
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "Mavi Krall�k (Jinno) botlar� olu�turuluyor...");
...
// Bot dosyalar�n� yeniden y�kle
ACMD(do_BotReload)
{
	if (!ch)
		return;

	ch->ChatPacket(CHAT_TYPE_INFO, "Bot dosyalar� yeniden y�kleniyor...");
	CBotCharacterManager::instance().Reload();
	ch->ChatPacket(CHAT_TYPE_INFO, "Bot dosyalar� ba�ar�yla yeniden y�klendi!");
	ch->ChatPacket(CHAT_TYPE_INFO, "Yeni botlar olu�turdu�unuzda g�ncel ayarlar kullan�lacak.");
}
#endif
