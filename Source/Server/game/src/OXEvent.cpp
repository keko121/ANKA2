...
bool COXEventManager::ShowQuizList(LPCHARACTER pkChar)
{
	...
	for (size_t i = 0; i < m_vec_quiz.size(); ++i)
	{
		for (size_t j = 0; j < m_vec_quiz[i].size(); ++j, ++c)
		{
			...
			{
				pkChar->LocaleChatPacket(CHAT_TYPE_INFO, 555, "%s", m_vec_quiz[i][j].Quiz);
			}
			else
			{
				pkChar->LocaleChatPacket(CHAT_TYPE_INFO, 556, "%s", m_vec_quiz[i][j].Quiz);
			}
		}
	}

	pkChar->LocaleChatPacket(CHAT_TYPE_INFO, 131, "%d", c);
	return true;
}
...
EVENTFUNC(oxevent_timer)
{
	...
	switch (flag)
	{
		case 0:
			...
			SendLocaleNotice(CHAT_TYPE_CONTROL_NOTICE, 0, OXEVENT_MAP_INDEX, 557, "");
#else
			SendLocaleNotice(CHAT_TYPE_BIG_NOTICE, 0, OXEVENT_MAP_INDEX, 557, "");
#endif
			flag++;
		...
		case 1:
#ifdef ENABLE_RENEWAL_OX_EVENT
			SendNoticeOxMap("#start", OXEVENT_MAP_INDEX);
#endif
			if (info->answer == true)
			{
				COXEventManager::instance().CheckAnswer(true);
#ifdef ENABLE_RENEWAL_OX_EVENT
				SendLocaleNotice(CHAT_TYPE_CONTROL_NOTICE, 0, OXEVENT_MAP_INDEX, 558, "");
#else
				SendLocaleNotice(CHAT_TYPE_BIG_NOTICE, 0, OXEVENT_MAP_INDEX, 558, "");
#endif
			}
			else
			{
...
#ifdef ENABLE_RENEWAL_OX_EVENT
				SendLocaleNotice(CHAT_TYPE_CONTROL_NOTICE, 0, OXEVENT_MAP_INDEX, 559, "");
#else
				SendLocaleNotice(CHAT_TYPE_BIG_NOTICE, 0, OXEVENT_MAP_INDEX, 559, "");
#endif
			}

#ifdef ENABLE_RENEWAL_OX_EVENT
			SendLocaleNotice(CHAT_TYPE_CONTROL_NOTICE, 0, OXEVENT_MAP_INDEX, 560, "");
			SendNoticeOxMap("#send", OXEVENT_MAP_INDEX);
#else
			SendLocaleNotice(CHAT_TYPE_BIG_NOTICE, 0, OXEVENT_MAP_INDEX, 560, "");
#endif
...
#ifdef ENABLE_RENEWAL_OX_EVENT
			SendNoticeOxMap("#end", OXEVENT_MAP_INDEX);
#else
			SendLocaleNotice(CHAT_TYPE_BIG_NOTICE, 0, OXEVENT_MAP_INDEX, 561, "");
#endif
			flag = 0;
			break;
	}
...
#ifdef ENABLE_RENEWAL_OX_EVENT
	SendLocaleNotice(CHAT_TYPE_CONTROL_NOTICE, 0, OXEVENT_MAP_INDEX, 562, "");
	SendNoticeOxMap(m_vec_quiz[level][idx].Quiz, OXEVENT_MAP_INDEX);
	SendLocaleNotice(CHAT_TYPE_CONTROL_NOTICE, 0, OXEVENT_MAP_INDEX, 563, "");
	SendNoticeOxMap("#start", OXEVENT_MAP_INDEX);
#else
	SendLocaleNotice(CHAT_TYPE_BIG_NOTICE, 0, OXEVENT_MAP_INDEX, 562, "");
	SendLocaleNotice(CHAT_TYPE_BIG_NOTICE, 0, OXEVENT_MAP_INDEX, std::stoi(m_vec_quiz[level][idx].Quiz), "");
	SendLocaleNotice(CHAT_TYPE_BIG_NOTICE, 0, OXEVENT_MAP_INDEX, 563, "");
#endif
...
bool COXEventManager::CheckAnswer(bool answer)
{
	...
	for (; iter != m_map_attender.end();)
	{
		...
		if (pkChar != NULL)
		{
			...
			{
				pkChar->LocaleChatPacket(CHAT_TYPE_INFO, 132, "");
