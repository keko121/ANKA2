...
typedef std::unordered_map<DWORD, std::string>					TCubeResultInfoTextByNPC;
...
void Cube_open (LPCHARACTER ch)
{
	...
	if (ch->IsCubeOpen())
	{
		ch->LocaleChatPacket(CHAT_TYPE_INFO, 52, "");
		return;
	}
...
#ifdef ENABLE_CUBE_GAME_CORE_FIX
		if (resultCount == 0) {
			return;
		}
#endif
...
#ifdef ENABLE_CUBE_REQUEST_LIST_FIX
