...
DWORD CGuildManager::CreateGuild(TGuildCreateParameter& gcp)
{
	...
	{
		gcp.master->LocaleChatPacket(CHAT_TYPE_INFO, 150, "");
		return 0;
	}
	...
	if (pmsg->Get()->uiNumRows > 0)
	{
		...
		if (!(row[0] && row[0][0] == '0'))
		{
			gcp.master->LocaleChatPacket(CHAT_TYPE_INFO, 151, "");
			return 0;
		}
	}
	else
	{
		gcp.master->LocaleChatPacket(CHAT_TYPE_INFO, 152, "");
		return 0;
	}
...
CGuild* CGuildManager::FindGuildByName(const char* cpszName)
{
	for (const auto& [k, v] : m_mapGuild)
	{
		if (std::strcmp(v->GetName(), cpszName) == 0)
		{
			return v;
		}
	}
...
{
	std::sort(v.begin(), v.end(), [](const CGuild* g1, const CGuild* g2) ->bool {
		...
		if (g1->GetGuildWarLossCount() > g2->GetGuildWarLossCount())
			return false;
		const int32_t c = strcmp(g1->GetName(), g2->GetName());
		if (c > 0)
			return true;
		return false;
		});
...
void CGuildManager::DeclareWar(DWORD guild_id1, DWORD guild_id2, BYTE bType)
{
		...
		g2->DeclareWar(guild_id1, bType, GUILD_WAR_RECV_DECLARE))
	{
		...
		{
			char buf[256];
			snprintf(buf, sizeof(buf),"160 %s %s", TouchGuild(guild_id1)->GetName(), TouchGuild(guild_id2)->GetName());
			SendNotice(buf);
		}
	}
}
...
void CGuildManager::RefuseWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1 && g2)
	{
		if (g2->GetMasterCharacter())
			g2->GetMasterCharacter()->LocaleChatPacket(CHAT_TYPE_INFO, 153, "%s", g1->GetName());
	}
...
void CGuildManager::WaitStartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	{
		char buf[256];
		snprintf(buf, sizeof(buf),"155 %s %s", g1->GetName(), g2->GetName());
		SendNotice(buf);
	}
}
...
void CGuildManager::StartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	char buf[256];
	snprintf(buf, sizeof(buf),"156 %s %s", g1->GetName(), g2->GetName());
	SendNotice(buf);
...
void SendGuildWarOverNotice(CGuild* g1, CGuild* g2, bool bDraw)
{
	if (g1 && g2)
	{
		...
		if (bDraw)
		{
			snprintf(buf, sizeof(buf),"157 %s %s", g1->GetName(), g2->GetName());
		}
		else
		{
			if ( g1->GetWarScoreAgainstTo( g2->GetID() ) > g2->GetWarScoreAgainstTo( g1->GetID() ) )
			{
				snprintf(buf, sizeof(buf),"158 %s %s", g1->GetName(), g2->GetName());
			}
			else
			{
				snprintf(buf, sizeof(buf),"158 %s %s", g2->GetName(), g1->GetName());
			}
		}
...
void CGuildManager::CancelWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1)
	{
		...
		if (master1)
			master1->LocaleChatPacket(CHAT_TYPE_INFO, 154, "");
	}
	...
	if (g2)
	{
		...
		if (master2)
			master2->LocaleChatPacket(CHAT_TYPE_INFO, 154, "");
	}

	if (g1 && g2)
	{
		char buf[256+1];
		snprintf(buf, sizeof(buf),"159 %s %s", g1->GetName(), g2->GetName());
		SendNotice(buf);
	}
}
...
void CGuildManager::ReserveWarAdd(TGuildWarReserve * p)
{
	...
	std::memcpy(&pkReserve->data, p, sizeof(TGuildWarReserve));
