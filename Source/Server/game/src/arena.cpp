...
EVENTFUNC(ready_to_start_event)
{
	...
	switch (info->state)
	{
		case 0:
			{
				...
				if (count > 10000)
				{
					chA->LocaleChatPacket(CHAT_TYPE_INFO, 2, "");
					chB->LocaleChatPacket(CHAT_TYPE_INFO, 2, "");
				}
				else
				{
					...
					chA->LocaleChatPacket(CHAT_TYPE_INFO, 3, "%d", chA->GetPotionLimit());
					chB->LocaleChatPacket(CHAT_TYPE_INFO, 3, "%d", chA->GetPotionLimit());
				}
				chA->LocaleChatPacket(CHAT_TYPE_INFO, 4, "");
				chB->LocaleChatPacket(CHAT_TYPE_INFO, 4, "");
				pArena->SendChatPacketToObserver(CHAT_TYPE_INFO, 4, "");
		...
		case 1:
			{
				chA->LocaleChatPacket(CHAT_TYPE_INFO, 5, "");
				chB->LocaleChatPacket(CHAT_TYPE_INFO, 5, "");
				pArena->SendChatPacketToObserver(CHAT_TYPE_INFO, 5, "");
		...
		default:
			{
				chA->LocaleChatPacket(CHAT_TYPE_INFO, 6, "");
				chB->LocaleChatPacket(CHAT_TYPE_INFO, 6, "");
				pArena->SendChatPacketToObserver(CHAT_TYPE_INFO, 6, "");
...
EVENTFUNC(duel_time_out)
{
	...
	else
	{
		...
		{
			case 0:
				pArena->SendChatPacketToObserver(CHAT_TYPE_NOTICE, 7, "");
				pArena->SendChatPacketToObserver(CHAT_TYPE_NOTICE, 8, "");

				chA->LocaleChatPacket(CHAT_TYPE_INFO, 7, "");
				chA->LocaleChatPacket(CHAT_TYPE_INFO, 8, "");

				chB->LocaleChatPacket(CHAT_TYPE_INFO, 7, "");
				chB->LocaleChatPacket(CHAT_TYPE_INFO, 8, "");
...
bool CArena::OnDead(DWORD dwPIDA, DWORD dwPIDB)
{
	...
	{
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, 9, "");
		restart = false;
	}
	else if (pCharA == NULL && pCharB != NULL)
	{
		pCharB->LocaleChatPacket(CHAT_TYPE_INFO, 10, "");
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, 11, "");
		restart = false;
	}
	else if (pCharA != NULL && pCharB == NULL)
	{
		pCharA->LocaleChatPacket(CHAT_TYPE_INFO, 10, "");
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, 11, "");
		restart = false;
	}
	else if (pCharA != NULL && pCharB != NULL)
	{
		if (m_dwPIDA == dwPIDA)
		{
			...
			{
				pCharA->LocaleChatPacket(CHAT_TYPE_NOTICE, 12, "%s", pCharA->GetName());
			...
			{
				restart = true;
				pCharA->LocaleChatPacket(CHAT_TYPE_INFO, 13, "%s", pCharA->GetName());
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());

				pCharB->LocaleChatPacket(CHAT_TYPE_INFO, 13, "%s", pCharA->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());

				SendChatPacketToObserver(CHAT_TYPE_NOTICE, 567, "%s#%d#%s#%d", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());
		...
		else if (m_dwPIDB == dwPIDA)
		{
			...
			{
				pCharA->LocaleChatPacket(CHAT_TYPE_NOTICE, 12, "%s", pCharB->GetName());
				pCharB->LocaleChatPacket(CHAT_TYPE_NOTICE, 12, "%s", pCharB->GetName());
				SendChatPacketToObserver(CHAT_TYPE_NOTICE, 12, "%s", pCharB->GetName());
			...
			{
				restart = true;
				pCharA->LocaleChatPacket(CHAT_TYPE_INFO, 13, "%s", pCharB->GetName());
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());

				pCharB->LocaleChatPacket(CHAT_TYPE_INFO, 13, "%s", pCharB->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());
	...
	{
		if (pCharA != NULL)
			pCharA->LocaleChatPacket(CHAT_TYPE_INFO, 8, "");

		if (pCharB != NULL)
			pCharB->LocaleChatPacket(CHAT_TYPE_INFO, 8, "");

		SendChatPacketToObserver(CHAT_TYPE_INFO, 8, "");
	...
	{
		if (pCharA != NULL)
			pCharA->LocaleChatPacket(CHAT_TYPE_INFO, 14, "");

		if (pCharB != NULL)
			pCharB->LocaleChatPacket(CHAT_TYPE_INFO, 14, "");

		SendChatPacketToObserver(CHAT_TYPE_INFO, 14, "");
...
void CArena::OnDisconnect(DWORD pid)
{
	...
	{
		if (GetPlayerB() != NULL)
			GetPlayerB()->LocaleChatPacket(CHAT_TYPE_INFO, 15, "");
	...
	{
		if (GetPlayerA() != NULL)
			GetPlayerA()->LocaleChatPacket(CHAT_TYPE_INFO, 15, "");
...
void CArena::SendChatPacketToObserver(BYTE type, DWORD id, const char* format, ...)
{
	char chatbuf[256];
	va_list args;
	...
	for (; iter != m_mapObserver.end(); iter++)
	{
		...
		if (pChar)
		{
			if (pChar->GetDesc() != NULL)
			{
				pChar->LocaleChatPacket(type, id, chatbuf);
			}
		}
	}
}
