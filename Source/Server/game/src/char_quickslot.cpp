...
bool CHARACTER::SetQuickslot(BYTE pos, TQuickslot & rSlot)
{
	...
	switch (rSlot.type)
	{
		case QUICKSLOT_TYPE_ITEM:
			...
			&& srcCell.IsSkillBookInventoryPosition() == false 
			&& srcCell.IsUpgradeItemsInventoryPosition() == false
			&& srcCell.IsStoneInventoryPosition() == false
			&& srcCell.IsGiftBoxInventoryPosition() == false
			&& srcCell.IsChangersInventoryPosition() == false
#endif
			)
				return false;
