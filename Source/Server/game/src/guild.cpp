...
void CGuild::RequestAddMember(LPCHARACTER ch, int grade)
{
	...
	const auto dwPID = ch->GetPlayerID();
...
#ifdef ENABLE_LEVEL_INT
	sys_log(0, "GUILD: AddMember PID %u, grade %u, job %u, level %d, offer %u, name %s ptr %p", p->dwPID, p->bGrade, p->bJob, p->bLevel, p->dwOffer, p->szName, get_pointer(ch));
#else
	sys_log(0, "GUILD: AddMember PID %u, grade %u, job %u, level %u, offer %u, name %s ptr %p", p->dwPID, p->bGrade, p->bJob, p->bLevel, p->dwOffer, p->szName, get_pointer(ch));
#endif
...
void CGuild::LoadGuildData(SQLMsg* pmsg)
{
	...
	if (row[5])
		std::memcpy(m_data.abySkill, row[5], sizeof(BYTE) * GUILD_SKILL_COUNT);
	else
...
void CGuild::SendDBSkillUpdate(int amount)
{
	...
	std::memcpy(guild_skill.skill_levels, m_data.abySkill, sizeof(BYTE) * GUILD_SKILL_COUNT);
...
BYTE CGuild::IsGuildGeneral(DWORD pid)
{
...
bool CGuild::OfferExp(LPCHARACTER ch, int amount)
{
	...
	{
		ch->LocaleChatPacket(CHAT_TYPE_INFO, 382, "");
		return false;
	}
	...
	{
		ch->LocaleChatPacket(CHAT_TYPE_INFO, 737, "");
		return false;
	}
#endif
...
struct FRefreshCommentForce {
    CGuild& guild;
    const uint32_t playerId;

    FRefreshCommentForce(CGuild& guild, const uint32_t playerId)
        : guild(guild), playerId(playerId) {}

    void operator()() { guild.RefreshCommentForce(playerId); }
};

void CGuild::AddComment(LPCHARACTER ch, const std::string& str)
{
    if (str.length() > GUILD_COMMENT_MAX_LEN || str.empty())
    {
        return;
    }
    ...
    DBManager::instance().FuncAfterQuery(
        FRefreshCommentForce(*this, ch->GetPlayerID()),
        "INSERT INTO guild_comment%s(guild_id, name, notice, content, time) "
        "VALUES(%u, '%s', %d, '%s', NOW())",
        get_table_postfix(), m_data.guild_id, ch->GetName(),
        (str[0] == '!') ? 1 : 0, text);
}
...
void CGuild::DeleteComment(LPCHARACTER ch, DWORD comment_id)
{
	...
	if (pmsg->Get()->uiAffectedRows == 0 || pmsg->Get()->uiAffectedRows == (uint32_t)-1)
		ch->LocaleChatPacket(CHAT_TYPE_INFO, 383, "");
	else
...
void CGuild::UseSkill(DWORD dwVnum, LPCHARACTER ch, DWORD pid)
{
	...
	if (GetSP() < iNeededSP)
	{
		ch->LocaleChatPacket(CHAT_TYPE_INFO, 384, "%d#%d", GetSP(), iNeededSP);
		return;
	}
	...
	{
		ch->LocaleChatPacket(CHAT_TYPE_INFO, 385, "");
		return;
	}
		...
		case GUILD_SKILL_TELEPORT:
			...
			else
			{
				if (m_memberP2POnline.find(pid) != m_memberP2POnline.end())
				{
					...
					{
						ch->LocaleChatPacket(CHAT_TYPE_INFO, 56, "%d#%d", pcci->bChannel, g_bChannel);
					}
					else
					{
		...
		default:
			{
				if (!UnderAnyWar())
				{
					ch->LocaleChatPacket(CHAT_TYPE_INFO, 386, "");
					return;
				}
...
void CGuild::UpdateSkill(BYTE skill_point, BYTE* skill_levels)
{
	...
	std::memcpy(m_data.abySkill, skill_levels, sizeof(BYTE) * GUILD_SKILL_COUNT);
	ComputeGuildPoints();
}
...
bool CGuild::ChargeSP(LPCHARACTER ch, int iSP)
{
	...
	{
		ch->LocaleChatPacket(CHAT_TYPE_INFO, 387, "%u", iSP);
	}
	return true;
}
...
void CGuild::RequestDepositMoney(LPCHARACTER ch, int iGold)
{
	...
	{
		ch->LocaleChatPacket(CHAT_TYPE_INFO, 388, "");
		return;
	}
...
void CGuild::RequestWithdrawMoney(LPCHARACTER ch, int iGold)
{
	...
	{
		ch->LocaleChatPacket(CHAT_TYPE_INFO, 389, "");
		return;
	}
...
void CGuild::Invite( LPCHARACTER pchInviter, LPCHARACTER pchInvitee )
{
	...
	{
		pchInviter->LocaleChatPacket(CHAT_TYPE_INFO, 392, "");
		return;
	}
	else if (pchInvitee->GetEmpire() != pchInviter->GetEmpire())
	{
		pchInviter->LocaleChatPacket(CHAT_TYPE_INFO, 393, "");
		return;
	}
	...
	switch (errcode)
	{
		...
		case GERR_WITHDRAWPENALTY:
			pchInviter->LocaleChatPacket(CHAT_TYPE_INFO, 394, "%d", quest::CQuestManager::instance().GetEventFlag("guild_withdraw_delay"));
			return;

		case GERR_COMMISSIONPENALTY:
			pchInviter->LocaleChatPacket(CHAT_TYPE_INFO, 395, "%d", quest::CQuestManager::instance().GetEventFlag("guild_disband_delay"));
			return;

		case GERR_ALREADYJOIN:
			pchInviter->LocaleChatPacket(CHAT_TYPE_INFO, 396, "");
			return;

		case GERR_GUILDISFULL:
			pchInviter->LocaleChatPacket(CHAT_TYPE_INFO, 397, "");
			return;

		case GERR_GUILD_IS_IN_WAR:
			pchInviter->LocaleChatPacket(CHAT_TYPE_INFO, 324, "");
			return;

		case GERR_INVITE_LIMIT:
			pchInviter->LocaleChatPacket(CHAT_TYPE_INFO, 398, "");
			return;
...
void CGuild::InviteAccept( LPCHARACTER pchInvitee )
{
	...
	switch ( errcode )
	{
		...
		case GERR_WITHDRAWPENALTY:
			pchInvitee->LocaleChatPacket(CHAT_TYPE_INFO, 394, "%d", quest::CQuestManager::instance().GetEventFlag("guild_withdraw_delay"));
			return;

		case GERR_COMMISSIONPENALTY:
			pchInvitee->LocaleChatPacket(CHAT_TYPE_INFO, 395, "%d", quest::CQuestManager::instance().GetEventFlag("guild_disband_delay"));
			return;

		case GERR_ALREADYJOIN:
			pchInvitee->LocaleChatPacket(CHAT_TYPE_INFO, 396, "");
			return;

		case GERR_GUILDISFULL:
			pchInvitee->LocaleChatPacket(CHAT_TYPE_INFO, 397, "");
			return;

		case GERR_GUILD_IS_IN_WAR:
			pchInvitee->LocaleChatPacket(CHAT_TYPE_INFO, 324, "");
			return;

		case GERR_INVITE_LIMIT:
			pchInvitee->LocaleChatPacket(CHAT_TYPE_INFO, 398, "");
			return;
