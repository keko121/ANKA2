...
EVENTINFO(war_map_info)
{
	...
	war_map_info() : iStep( 0 ) , pWarMap( 0 ) {}
};
...
{
	set_pidJoiner.erase(ch->GetPlayerID()); // Fix
	--iMemberCount;
}
...
void CWarMap::IncMember(LPCHARACTER ch)
{
	...
	else
	{
		...
		ch->LocaleChatPacket(CHAT_TYPE_INFO, 114, "");
	}
...
void CWarMap::CheckWarEnd()
{
	...
	if (m_TeamData[0].iMemberCount == 0 || m_TeamData[1].iMemberCount == 0)
	{
		...
		Notice(CHAT_TYPE_NOTICE, 549, "");
		Notice(CHAT_TYPE_NOTICE, 550, "");
...
namespace
{
	...
	struct FNotice
	{
		...
		FNotice(BYTE type, DWORD id, const char * format) : m_type(type), m_id(id), m_format(format) {}

		void operator() (LPCHARACTER ch)
		{
			ch->LocaleChatPacket(m_type, m_id, m_format);
		}
	};
};

void CWarMap::Notice(BYTE type, DWORD id, const char* format, ...)
{
	char chatbuf[256];
	va_list args;
