...
namespace quest
{
	...
	struct FPurgeArea
	{
		...
		FPurgeArea(int a, int b, int c, int d, LPCHARACTER p): x1(a), y1(b), x2(c), y2(d), ExceptChar(p) {}
	...
	{
		int GetX,GetY;
		FGlobalWarpAllToBase(int x, int y) : GetX(x), GetY(y) {}
		void operator()(LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
	...
	EVENTINFO(global_warp_all_to_base_event_info)
	{
		...
		int y;
		int MapIndex;
		global_warp_all_to_base_event_info() : x(0), y(0), MapIndex(0) {}
	};
	...
	{
		int item;
		FRemove_All(int needitem) : item(needitem) {}
		void operator () (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
	...
	struct FKillArea
	{
		int x1, y1, x2, y2;
		LPCHARACTER ExceptChar;
		FKillArea(int a, int b, int c, int d, LPCHARACTER p) : x1(a), y1(b), x2(c), y2(d), ExceptChar(p) {}
		void operator () (LPENTITY ent)
		{
			if (true == ent->IsType(ENTITY_CHARACTER))
			{
	...
	int dungeon_kill_area(lua_State* L)
	{
		if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2) || !lua_isnumber(L, 3) || !lua_isnumber(L, 4))
			return 0;
	...
	{
		int GetVid,GetY;
		FGetMonsterHP(int vid) : GetVid(vid) {}
