...
bool CItemIDRangeManager::BuildRange(DWORD dwMin, DWORD dwMax, TItemIDRangeTable& range)
{
	...
	DWORD dwItemMaxOffShopID = 0;
	snprintf(szQuery, sizeof(szQuery), "SELECT MAX(id) FROM offline_shop_item%s WHERE id >= %u and id <= %u", GetTablePostfix(), dwMin, dwMax);
	SQLMsg* pMsgOffShop = CDBManager::instance().DirectQuery(szQuery);

	if (pMsgOffShop != NULL)
	{
		if (pMsgOffShop->Get()->uiNumRows > 0)
		{
			row = mysql_fetch_row(pMsgOffShop->Get()->pSQLResult);
			str_to_number(dwItemMaxOffShopID, row[0]);
		}
		delete pMsgOffShop;
	}

	if(dwItemMaxOffShopID > dwItemMaxID)
		dwItemMaxID = dwItemMaxOffShopID;
#endif
