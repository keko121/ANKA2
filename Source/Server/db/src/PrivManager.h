...
struct TPrivEmpireData
{
	...
	TPrivEmpireData(BYTE type, int value, BYTE empire, time_t end_time_sec) : type(type), value(value), bRemoved(false), empire(empire), end_time_sec(end_time_sec) {}
};
...
struct TPrivGuildData
{
	...
	TPrivGuildData(BYTE type, int value, DWORD guild_id, time_t _end_time_sec) : type(type), value(value), bRemoved(false), guild_id(guild_id), end_time_sec(_end_time_sec ) {}
};
...
struct TPrivCharData
{
	...
	TPrivCharData(BYTE type, int value, DWORD pid): type(type), value(value), bRemoved(false), pid(pid) {}
};
...
class CPrivManager : public singleton<CPrivManager>
{
	...
	private:
		...
		std::priority_queue<stPairChar, std::vector<stPairChar>, std::greater<stPairChar> > m_pqPrivChar;
		std::priority_queue<stPairGuild, std::vector<stPairGuild>, std::greater<stPairGuild> > m_pqPrivGuild;
