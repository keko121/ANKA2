...
void CActorInstance::ReservingMotionProcess()
{
	...
	switch (dwNextMotionIndex)
	{
		...
		case CRaceMotionData::NAME_STAND_UP_BACK:
			if (IsFaint())
			{
				//Tracenf("ÀÏ¾î¼­·Á°í ÇßÀ¸³ª ±âÀ?Áß");

				SetEndStopMotion();

				// ÀÌÈÄÀÇ ¸?¼Ç ÀüºÎ 1ÃÊ¾¿ µô·¹ÀÌ
	...
	// FIX: À§¿¡¼­ È£ÃâÇÑ __SetMotion ÇÔ¼ö ¾È¿¡¼­ ·£´?À¸·Î ´Ù¸¥ ¸?¼ÇÀ» Àç»?ÇÒ °¡´É¼ºµµ ÀÖÀ¸¹Ç·Î durationÀº 'ÇöÀç Àç»?ÁßÀÎ' ¸?¼ÇÀÇ duration°ªÀ» »ç¿ëÇØ¾ß ÇÔ.
...
{
	if (MOTION_TYPE_LOOP == m_kCurMotNode.iMotionType) // ÀÓ½Ã´Ù. ÃÖÁ¾ÀûÀÎ ¸ñÇ¥´Â Onceµµ À?´ë·Î ³Ñ¾î°¡¼± ¾ÈµÈ´Ù. - [levites]
		if (m_kCurMotNode.dwcurFrame >= m_kCurMotNode.dwFrameCount)
			m_kCurMotNode.dwcurFrame = 0;
...
BOOL CActorInstance::isLock()
{
	...
	switch (dwCurMotionIndex)
	{
		...
		case CRaceMotionData::NAME_EMOTION_1:
		case CRaceMotionData::NAME_EMOTION_2:
		case CRaceMotionData::NAME_EMOTION_3:
		case CRaceMotionData::NAME_EMOTION_4:
		case CRaceMotionData::NAME_EMOTION_5:
		case CRaceMotionData::NAME_EMOTION_6:
		case CRaceMotionData::NAME_EMOTION_7:
		case CRaceMotionData::NAME_EMOTION_8:
		case CRaceMotionData::NAME_EMOTION_9:
		case CRaceMotionData::NAME_EMOTION_10:
		case CRaceMotionData::NAME_EMOTION_11:
		case CRaceMotionData::NAME_EMOTION_12:
		case CRaceMotionData::NAME_EMOTION_13:
		case CRaceMotionData::NAME_EMOTION_14:
		case CRaceMotionData::NAME_EMOTION_15:
		case CRaceMotionData::NAME_EMOTION_16:
		case CRaceMotionData::NAME_EMOTION_17:
		case CRaceMotionData::NAME_EMOTION_18:
		case CRaceMotionData::NAME_EMOTION_19:
		case CRaceMotionData::NAME_EMOTION_20:
		case CRaceMotionData::NAME_EMOTION_21:
...
{
	// NOTE : ÀÚÁÖ È£Ãâ µÇ´Â ºÎº?Àº ¾Æ´ÏÁö¸¸ ¾î´À Á¤µµÀÇ ÃÖÀûÈ­ ¿©Áö°¡ ÀÖÀ½ - [levites]
	// FIXME : Ã³À½¿¡ ¼±ÅÃµÈ ¸?¼ÇÀÌ ¾ø´Â °Í¿¡ ´ëÇÑ Ã³¸®°¡ µÇ¾î ÀÖÁö ¾Ê´Ù.
...
DWORD CActorInstance::__SetMotion(const SSetMotionData& c_rkSetMotData, DWORD dwRandMotKey)
{
	...
	if (m_pkHorse)
	{
		...
		if (c_rkSetMotData.iLoopCount)
			m_pkHorse->m_kCurMotNode.iMotionType = MOTION_TYPE_ONCE; // ¹«Á¶°Ç ÀÌÀü ¸?¼Ç Å¸ÀÔÀ¸·Î ¼³Á¤µÇ°í ÀÖ¾úÀ½
		else
	...
	if (__BindMotionData(dwMotKey))
	{
		...
		if (__CanAttack())
		{
			// ¿©±â¼­ °ø°? ¸?¼ÇÀÏ °æ¿ìÀÇ Ã³¸®¸¦ ÇÕ´Ï´Ù - [levites]
			__ShowWeaponTrace();
		...
		if (__IsComboAttacking())
		{
			...
			{
				// 2004.11.19.myevan.µ¿¹° º¯½Å½Ã ÀÌºÎº?¿¡¼­ ¹Ù·Î ¸®¼ÂµÇ¾î ´ÙÀ½µ¿ÀÛ ¾È³ª¿Â´Ù
				m_dwcurComboIndex = 0; // Ä?º¸ ¸®¼Â - [levites]

				// NOTE : ClearCombo() ¸¦ ¼öÇàÇØ¼­´Â ¾ÈµÈ´Ù.
				//        Ä?º¸ ´ÙÀ½¿¡ ½ºÅ³À» ÀÌ¾î¼­ »ç¿ëÇÒ °æ¿ì m_pkCurRaceMotionData±îÁö ÃÊ±âÈ­ µÇ¾î ¹ö¸°´Ù.
				//Tracef("MotionData¿¡ Ä?º¸ µ¥ÀÌÅ¸°¡ µé¾î ÀÖÁö ¾Ê½À´Ï´Ù.\n");
			}
		}
	}
