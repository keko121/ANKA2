...
float CMapManager::GetHeight(float fx, float fy)
{
	if (!m_pkMap)
	{
		TraceError("CMapManager::GetHeight(%f, %f) - 맵이 �?성되지 않은 상태에서 접근", fx, fy);
		return 0.0f;
	}
...
float CMapManager::GetTerrainHeight(float fx, float fy)
{
	if (!m_pkMap)
	{
		TraceError("CMapManager::GetTerrainHeight(%f, %f) - 맵이 �?성되지 않은 상태에서 접근", fx, fy);
		return 0.0f;
	}
...
bool CMapManager::GetWaterHeight(int iX, int iY, long * plWaterHeight)
{
	if (!m_pkMap)
	{
		TraceError("CMapManager::GetTerrainHeight(%f, %f) - 맵이 �?성되지 않은 상태에서 접근", iX, iY);
		return false;
	}
...
void CMapManager::BeginEnvironment()
{
	...
	if (mc_pcurEnvironmentData->bFogEnable)
	{
		...
		if (mc_pcurEnvironmentData->bDensityFog)
		{
			...
			STATEMANAGER.SetRenderState(D3DRS_FOGDENSITY, *((DWORD *) &fDensity));
		}
		else
		{
			...
			STATEMANAGER.SetRenderState(D3DRS_FOGSTART, *((DWORD *) &fFogNear));
			STATEMANAGER.SetRenderState(D3DRS_FOGEND, *((DWORD *) &fFogFar));
		}
	}
