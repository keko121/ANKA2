...
protected:
	...
	static BOOL SetupCompartmentSinks( BOOL bResetOnly = FALSE, ITfCompartment* pTfOpenMode = nullptr, ITfCompartment* ppTfConvMode = NULL );
...
int CIME::GetReading(std::string & rstrText)
{
	...
	if(ms_wstrReading.size() == 0)
		return 0;
	int readingLen = WideCharToMultiByte(ms_uOutputCodePage, 0, &ms_wstrReading[0], ms_wstrReading.size(), reading, sizeof(reading), nullptr, nullptr);
...
int CIME::GetText(std::string & rstrText, bool addCodePage)
{
	...
	len += WideCharToMultiByte(dataCodePage, 0, m_wText, ms_curpos, text, sizeof(text)-len, nullptr, nullptr);
	len += WideCharToMultiByte(dataCodePage, 0, m_wszComposition, ms_compLen, text+len, sizeof(text)-len, nullptr, nullptr);
	len += WideCharToMultiByte(dataCodePage, 0, m_wText+ms_curpos, ms_lastpos-ms_curpos, text+len, sizeof(text)-len, nullptr, nullptr);
...
void CIME::FinalizeString(bool bSend)
{
	...
	{
		//// Send composition std::string to app.
	...
	//InitCompStringData();
	// clear composition std::string in IME
	...
	// the following line is necessary as Korean IME doesn't close cand list when comp std::string is cancelled.
...
void CIME::SetSupportLevel( DWORD dwImeLevel )
{
	...
	// cancel current composition std::string.
	FinalizeString();
	//SetCompStringColor();
}
...
{
	int textLen = WideCharToMultiByte(ms_uOutputCodePage, 0, m_wText, ms_lastpos, 0, 0, nullptr, nullptr);
...
void CIME::CandidateProcess(HIMC hImc)
{
	...
	DWORD dwCandidateLen = ImmGetCandidateListW(hImc, 0, nullptr, 0);
...
void CIME::ReadingProcess(HIMC hImc)
{
    ...
    if (_GetReadingString)
	{
        UINT uMaxUiLen;
        BOOL bVertical;
        // Obtain the reading std::string size
		...
		if(tempLen == 0) {
			...
			if(bUnicodeIme) {
				...
				int wstrLen = MultiByteToWideChar(ms_uInputCodePage, 0, (char *)temp, tempLen, nullptr, 0);
				wchar_t* wstr = (wchar_t*)alloca(sizeof(wchar_t)*wstrLen);
...
bool CIME::IsMax(const wchar_t* wInput, int len)
{
	...
	//Sergio-2 max hyperlink in chat!
	const std::wstring inputText = wInput;
	if (inputText.find(L"|Hitem:") != std::wstring::npos)
	{
		std::string editLineText("");
		GetText(editLineText, true);
		for (int i = 0, count = 0; (i = editLineText.find("|Hitem:", i)) != std::string::npos; i++) {
			if (++count >= 2)
				return true;
		}
	}
	...
	int textLen = WideCharToMultiByte(ms_uOutputCodePage, 0, m_wText, ms_lastpos, 0, 0, nullptr, nullptr);
	int inputLen = WideCharToMultiByte(ms_uOutputCodePage, 0, wInput, len, 0, 0, nullptr, nullptr);
	...
	else if (m_userMax != 0 && m_max != m_userMax)
	{
		...
		int textLen = WideCharToMultiByte(ms_uOutputCodePage, 0, str.c_str(), str.length(), 0, 0, nullptr, nullptr);
		int inputLen = WideCharToMultiByte(ms_uOutputCodePage, 0, input.c_str(), input.length(), 0, 0, nullptr, nullptr);
