...
struct SGuildMarkBlock
{
	...
	BYTE 	m_abCompBuf[MAX_COMP_SIZE];	// ¾?ÃàµÈ µ¥ÀÌÅÍ
	size_t	m_sizeCompBuf;	// ¾?ÃàµÈ Å©±â
	DWORD	m_crc;			// ¾?ÃàµÈ µ¥ÀÌÅÍÀÇ CRC
...
class CGuildMarkImage
{
	public:
		...
		bool SaveBlockFromCompressedData(DWORD posBlock, const BYTE * pbComp, DWORD dwCompSize); // ¼­¹ö -> Å¬¶óÀÌ¾?Æ®
