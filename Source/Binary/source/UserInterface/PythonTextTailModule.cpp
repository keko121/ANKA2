...
PyObject *textTailRegisterCharacterTextTail(PyObject *poSelf, PyObject *poArgs)
{
	...
	int iGuildLeader;
	if (!PyTuple_GetInteger(poArgs, 1, &iGuildLeader))
		return Py_BuildException();
#endif
	int iVirtualID;
...
#ifdef ENABLE_GUILD_LEADER_TEXTAIL
	CPythonTextTail::Instance().RegisterCharacterTextTail(iGuildID, iVirtualID, iGuildLeader, D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));
#else
