...
PyObject *chatSetFontNameChat (PyObject *poSelf, PyObject *poArgs)
{
	char *szFontName;
	...
	CPythonChat::Instance().SetFontName (szFontName);
	return Py_BuildNone();
}

PyObject *chatSaveEmojisChat (PyObject *poSelf, PyObject *poArgs)
{
	char *szEmojisChat;
	if (!PyTuple_GetString (poArgs, 0, &szEmojisChat))
	{
		return Py_BuildException();
	}

	CPythonChat::Instance().SaveEmojisChat (szEmojisChat);
	return Py_BuildNone();
}
#endif
...
#ifdef ENABLE_EMOTICONS_SYSTEM
		{ "SetFontNameChat",		chatSetFontNameChat,		METH_VARARGS },
		{ "SaveEmojisChat",			chatSaveEmojisChat,			METH_VARARGS },
#endif
