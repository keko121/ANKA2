...
bool CPythonSkill::RegisterPetSkillTable(const char *c_szFileName)
{
	...
	for (DWORD i = 0; i < textFileLoader.GetLineCount(); ++i)
	{
		...
		m_PetSkillDataMap.insert(std::make_pair(iSkillIndex, TPetSkillData()));
...
bool CPythonSkill::RegisterSkill(DWORD dwSkillIndex, const char *c_szFileName)
{
	...
	{
		char szName[256];
		sprintf(szName, "%dname", GetCodePage());
	...
	{
		char szName[256];
		sprintf(szName, "%ddescription", GetCodePage());
...

	{
		...
		sprintf(szConditionData, "%dconditiondata", GetCodePage());
...

	{
		...
		sprintf(szAffectData, "%daffectdata", GetCodePage());
...

	{
		...
		sprintf(szGradeData, "%dgradedata", GetCodePage());
...
PyObject *skillGetEmptySkill(PyObject *poSelf, PyObject *poArgs)
{
	CGraphicImage* noskillbtn;
	char szIconFileNameHeader[64 + 1];
	_snprintf(szIconFileNameHeader, sizeof(szIconFileNameHeader), "d:/ymir work/ui/game/windows/skill_enable_button.sub");
...
PyObject *skillGetSkillPowerByLevel(PyObject *poSelf, PyObject *poArgs)
{
	...
	return Py_BuildValue("i", GetSkillPower(dwSkillLevel));
}
...
void initskill()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetEmptySkill", skillGetEmptySkill, METH_VARARGS },
