...
void Render(int Width, int Height)
{
    ...
    if(SUCCEEDED(g_pD3DDevice->BeginScene()))
    {
        ...
        for (size_t Idx = 0; Idx < GlobalScene.Models.size(); Idx++)
        {
            ...
            granny_model_instance* Instance = Model->ModelInstance;
            granny_skeleton* Skeleton = GrannyGetSourceSkeleton(Instance);

            GrannySampleModelAnimations(Instance, 0, Skeleton->BoneCount, GlobalScene.SharedLocalPose);
            GrannyBuildWorldPose(Skeleton, 0, Skeleton->BoneCount,
