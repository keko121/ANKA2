...
FLOAT __stdcall DXUtil_Timer( TIMER_COMMAND command )
{
    ...
    static BOOL     m_bUsingQPF         = TRUE;
    static BOOL     m_bTimerStopped     = TRUE;
    static LONGLONG m_llQPFTicksPerSec  = 0;

	static granny_system_clock StartClock;
    ...
    if( FALSE == m_bTimerInitialized )
    {
		...
		QueryPerformanceFrequency((LARGE_INTEGER*)&m_llQPFTicksPerSec);
    ...
    {
        // Get the time using granny's timer
        ...
        else
		{
			...
			fTime = GrannyGetSecondsElapsed(&StartClock, &CurrClock);
		}
