...
class PickMoneyDialog(ui.ScriptWindow):
	...
	def SetMaxTextInfo(self, flag):
		self.removeMaxText = flag
	...
	def Open(self, maxValue, unitValue=1):
		...
		if not self.removeMaxText:
			self.maxValueTextLine.SetText(str(localeInfo.NumberToMoneyString(maxValue)))
		else:
			self.maxValueTextLine.SetText(str(localeInfo.NumberToDecimalString(maxValue)))
